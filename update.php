...
} else {   /* Se ha pulsado el botón 'dar de baja' */

   /* Las tablas hijas (asignaturas) de la tabla alumnos se crearon con las */
   /* opciones ON DELETE CASCADE ON UPDATE CASCADE, por lo que al borrar    */
   /* una entrada en alumnos, se borrará también en las tablas vinculadas.  */ 

   $query = "DELETE FROM ".$table." WHERE id=".$_POST['id'];
   debug_msg($query);

   mysql_query($query, $link)
      or die("Error DELETE: ".mysql_error()); 

   echo "Se ha procedido a dar de baja al alumno en la base de datos.<br/>";

   /* Si volvemos al script anterior obtendríamos un error, ya que el id */
   /* del alumno no existe ya en la base de datos. Lo mejor es volver a  */
